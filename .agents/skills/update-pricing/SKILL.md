...
AWS Pricing API — do NOT scrape pricing pages. Instead:

1. Run the script without `--write` (it prints the generated `cost.py` to
   stdout). It requires AWS credentials — e.g. `AWS_PROFILE=dev` after an SSO
   login; if credentials are missing, report `FETCH_FAILED` and tell the user
   to log in.
2. Diff the generated output against the current `cost.py` and report the
   differences as Price Updates / New Models.
