...
class TestChat:
    ...
    def test_non_json_body_mapped(self, sync_provider: OpenAIProvider, respx_mock: respx.MockRouter) -> None:
...
class TestAchat:
    ...
    async def test_non_json_body_mapped(self, async_provider: OpenAIProvider, respx_mock: respx.MockRouter) -> None:
        respx_mock.post(_CHAT_URL).mock(return_value=httpx.Response(200, content=b"not json"))
        with pytest.raises(ProviderError):
            await async_provider.achat(MODEL, [UserMessage(content="Hi")])
...
class TestChatStream:
    ...
    def test_mid_stream_error_raises_after_partial(
        ...
        respx_mock.post(_CHAT_URL).mock(return_value=httpx.Response(200, content=sse))
...
class TestAchatStream:
    ...
    async def test_mid_stream_error_raises_after_partial(
        self, async_provider: OpenAIProvider, respx_mock: respx.MockRouter
    ) -> None:
        first = {"model": MODEL, "choices": [{"index": 0, "finish_reason": None, "delta": {"content": "Hel"}}]}
        error = {"error": {"message": "mid-stream boom"}}
        sse = (f"data: {json.dumps(first)}\n\ndata: {json.dumps(error)}\n\n").encode()
