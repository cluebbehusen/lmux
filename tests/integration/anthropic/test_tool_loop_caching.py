"""Anthropic prompt caching across the hops of an agentic tool loop: a cache point placed
after the tool results caches the accumulated tool history, not just the question.
...
# whatever precedes it — here, the tool results.
...
def test_cache_point_after_tool_results(
    ...
    def _weather(auth: Any, transport: Any) -> ChatResponse:  # noqa: ANN401 — harness-supplied per mode
        return AnthropicProvider(auth=auth, transport=transport).chat(_MODEL, ask, tools=_TOOLS, max_tokens=_MAX_TOKENS)

    weather = scenario(_WEATHER_CASSETTE, _weather, requires="ANTHROPIC_API_KEY")
    ...
    forecast = scenario(_FORECAST_CASSETTE, _forecast, requires="ANTHROPIC_API_KEY")
    ...
    assert tool_prefix > 0  # the tool results extend the cached prefix
    ...
    answer = scenario(_ANSWER_CASSETTE, _answer, requires="ANTHROPIC_API_KEY")
    assert_chat(answer, provider="anthropic")
