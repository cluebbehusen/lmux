"""Anthropic streaming: the streamed content is the expected word, and the terminal
...
def _stream(auth: Any, transport: Any) -> list[ChatChunk]:  # noqa: ANN401 — harness-supplied per mode
    provider = AnthropicProvider(auth=auth, transport=transport)
...
def test_streaming(scenario: Callable[..., Any], assert_cost: Callable[..., None]) -> None:
    chunks = scenario(_CASSETTE, _stream, requires="ANTHROPIC_API_KEY")
