"""Bedrock Converse prompt caching across the hops of an agentic tool loop: a cache point
placed after the tool results caches the accumulated tool history, not just the question.

Three hops are the fewest that show this. Two cannot distinguish caching the question from
caching the tool history — both read back the same prefix. The third hop reads a prefix the
previous hop's tool result extended, so a strictly larger read is what proves the tool result
sits inside the cached prefix. The tool results of a parallel-call turn merge into a single
Converse user message, so this also covers a ``cachePoint`` placed after a merged turn.
"""
...
_WEATHER_CASSETTE = _CASSETTES / "tool_loop_cache_weather.json"
_FORECAST_CASSETTE = _CASSETTES / "tool_loop_cache_forecast.json"
_ANSWER_CASSETTE = _CASSETTES / "tool_loop_cache_answer.json"
...
# A marker-only message: it contributes no content of its own, so the cachePoint lands on
...
def test_cache_point_after_tool_results(
    ...
    weather = scenario(_WEATHER_CASSETTE, _weather, requires="AWS_BEARER_TOKEN_BEDROCK", offline_auth=offline_auth)
    ...
    forecast = scenario(_FORECAST_CASSETTE, _forecast, requires="AWS_BEARER_TOKEN_BEDROCK", offline_auth=offline_auth)
    ...
    answer = scenario(_ANSWER_CASSETTE, _answer, requires="AWS_BEARER_TOKEN_BEDROCK", offline_auth=offline_auth)
    assert_chat(answer, provider="aws-bedrock")
